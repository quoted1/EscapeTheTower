...
public class RouteSquare_10 : Puzzle_02_Controller {

    public void Pipe_Check()
    {
        if (this.transform.eulerAngles.z >= 359 || this.transform.eulerAngles.z <= 1)
        {
            GameObject.Find("Puzzle_02").GetComponent<Puzzle_02_Controller>().routeCheck[9] = true;
        ...
        {
            GameObject.Find("Puzzle_02").GetComponent<Puzzle_02_Controller>().routeCheck[9] = false;
            Debug.Log("pipewrong");
        }
    }
